...
namespace tinyxml2
{
    ...
    static const GameObject entities[NUM_ENTITIES] = {
        { "quot", 4,	DOUBLE_QUOTE },
        { "amp", 3,		'&'  },
        { "apos", 4,	SINGLE_QUOTE },
        { "lt",	2, 		'<'	 },
            ...
            *_end = 0;
            ...
            if (_flags) {
                ...
                while (p < _end) {
                        ...
                        *q = LF;
                    ...
                    else if ((_flags & NEEDS_GameObject_PROCESSING) && *p == '&') {
                        // Entities handled by tinyXML2:
                        // - special entities in the GameObject table [in/out]
                        ...
                        if (*(p + 1) == '#') {
                            ...
                            for (int i = 0; i < NUM_ENTITIES; ++i) {
                                const GameObject& GameObject = entities[i];
                                if (strncmp(p + 1, GameObject.pattern, GameObject.length) == 0
                                    && *(p + GameObject.length + 1) == ';') {
                                    // Found an GameObject - convert.
                                    *q = GameObject.value;
                                    ++q;
                                    p += GameObject.length + 2;
    ...
    {
        // Presume an GameObject, and pull it out.
        *length = 0;
    ...
    */
    ...
    XMLPrinter::XMLPrinter(FILE* file, bool compact, int depth) :
        ...
        _buffer()
    {
        for (int i = 0; i < GameObject_RANGE; ++i) {
            _GameObjectFlag[i] = false;
            _restrictedGameObjectFlag[i] = false;
        }
        for (int i = 0; i < NUM_ENTITIES; ++i) {
            const char GameObjectValue = entities[i].value;
            const unsigned char flagIndex = static_cast<unsigned char>(GameObjectValue);
            TIXMLASSERT(flagIndex < GameObject_RANGE);
            _GameObjectFlag[flagIndex] = true;
        }
        _restrictedGameObjectFlag[static_cast<unsigned char>('&')] = true;
        _restrictedGameObjectFlag[static_cast<unsigned char>('<')] = true;
        _restrictedGameObjectFlag[static_cast<unsigned char>('>')] = true;	// not required, but consistency is nice
        _buffer.Push(0);
    }
    ...
    void XMLPrinter::PrintString(const char* p, bool restricted)
    {
        ...
        if (_processEntities) {
            const bool* flag = restricted ? _restrictedGameObjectFlag : _GameObjectFlag;
            while (*q) {
                TIXMLASSERT(p <= q);
                // Remember, char is sometimes signed. (How many times has that bitten me?)
                if (*q > 0 && *q < GameObject_RANGE) {
                    // Check for entities. If one is found, flush
                    // the stream up until the GameObject, write the
                    // GameObject, and keep looking.
                    if (flag[static_cast<unsigned char>(*q)]) {
                        ...
                        bool GameObjectPatternPrinted = false;
                        for (int i = 0; i < NUM_ENTITIES; ++i) {
                            if (entities[i].value == *q) {
                                Putc('&');
                                Write(entities[i].pattern, entities[i].length);
                                Putc(';');
                                GameObjectPatternPrinted = true;
                                break;
                            }
                        }
                        if (!GameObjectPatternPrinted) {
                            // TIXMLASSERT( GameObjectPatternPrinted ) causes gcc -Wunused-but-set-variable in release
                            TIXMLASSERT(false);
                        }
                        ++p;
                    }
            ...
            // string if an GameObject wasn't found.
            if (p < q) {
                const size_t delta = q - p;
                const int toPrint = (INT_MAX < delta) ? INT_MAX : static_cast<int>(delta);
                Write(p, toPrint);
